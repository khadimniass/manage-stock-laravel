    ...
    <x-div-card>
        ...
        <form action="{{ route('Entree.update', $entree->id) }}" method="post">
            ...
            <div>
                ...
                <x-input id="quantite" class="block mt-1 w-full" type="number" name="quantite" :value="old('quantite', $entree->quantite)" required autofocus />
            </div>

            <div>
                <x-label for="prix" :value="__('Prix')" />

                <x-input id="prix" class="block mt-1 w-full" type="number" name="prix" :value="old('prix', $entree->prix)" required autofocus />
            </div>

            <div>
                <x-label for="date" :value="__('Date')" />

                <x-input id="date" class="block mt-1 w-full" type="date" name="date" :value="old('date', $entree->date)" required autofocus />
            </div>
